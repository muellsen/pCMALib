...
*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
	   ...
	   proc = ''
#endif	   
       CALL SYSTEM(' date +%S%M%H%j > zbql1234_'// trim(adjustl(proc))
     +   //'.tmp')
*
*       Try all file numbers for LFLNO to 999 
*
       FILNO = LFLNO
 10    OPEN(FILNO,FILE='zbql1234_'//trim(adjustl(proc))//'.tmp',ERR=11)
       GOTO 12
 11    FILNO = FILNO + 1
 ...
 12    READ(FILNO,'(3(I2),I3)') SS,MM,HH,DD
       CLOSE(FILNO)
       CALL SYSTEM('rm zbql1234_'//trim(adjustl(proc))//'.tmp')
